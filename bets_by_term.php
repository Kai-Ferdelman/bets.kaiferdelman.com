...
$termFromForm = $_GET["search"];

echo "<br><h2>Open bets including: " . $termFromForm . "</h2><br>";
$sql = "SELECT id, bet, pro, con, prize, date, done FROM bets WHERE done=0 AND bet LIKE '%" . $termFromForm . "%'  AND excluded NOT LIKE '%" . $_SESSION['user_id'] . "%' AND (" . $privateGroupsString . ")";
$result = $mysqli->query($sql);
...
//fetch finished bets
echo "<br><h2>Closed Bets including: " . $termFromForm . "</h2><br>";
$sql = "SELECT id, bet, pro, con, prize, date, done FROM bets WHERE done=1 AND bet LIKE '%" . $termFromForm . "%'  AND excluded NOT LIKE '%" . $_SESSION['user_id'] . "%' AND (" . $privateGroupsString . ")";
$result = $mysqli->query($sql);
