...
echo "<br><h2>Open bets for " . $username . "</h2><br>";
$sql = "SELECT id, bet, pro, con, prize, date, done FROM bets WHERE done=0 AND (bet LIKE '%" . $username . "%' OR pro LIKE '%" . $username . "%' OR con LIKE '%" . $username . "%')  AND excluded NOT LIKE '%" . $_SESSION['user_id'] . "%' AND (" . $privateGroupsString . ")";
$result = $mysqli->query($sql);
...
//fetch finished bets
echo "<br><h2>Closed Bets for " . $username . "</h2><br>";
$sql = "SELECT id, bet, pro, con, prize, date, done FROM bets WHERE done=1 AND (bet LIKE '%" . $username . "%' OR pro LIKE '%" . $username . "%' OR con LIKE '%" . $username . "%')  AND excluded NOT LIKE '%" . $_SESSION['user_id'] . "%' AND (" . $privateGroupsString . ")";
$result = $mysqli->query($sql);
